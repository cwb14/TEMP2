...
$echo 6 "\t-U ratio/map_quality\tThe ratio cutoff [0-1] between the second best alignment score and the best alignment score to judge if a read is uniquely mapped. Or map quality cutoff (1-255] to determine if a read is uniquely mapped. By default, if XS tag is available in the alignment/bam file, the ratio mode is enabled and 0.8 is used; otherwise, the map_quality mode is enabled and 5 is used. 
The second best alignment score is determined by the XS tag output by bwa; for alignment with the XS tag, please use a integer map quality "
